...
// Semua yang mereturn promise menggunakan fungsi createAsyncThunk

// Fungsi untuk mengambil semua data task
export const fetchTasks = createAsyncThunk(
  "task/fetchTasks",
  async (params) => {
    const response = await axios.get(
      "https://pm-todo-api.informatika-itts.my.id/api/tasks",
      { params }
    );
...
// Fungsi untuk menambahkan data task
export const storeTask = createAsyncThunk(
  ...
  async (payload, { dispatch }) => {
    try {
      ...
      // Jika response API sukses maka load ulang data task dengan memanggil fungsi fetchTasks
      if (results.success) {
...
// Fungsi untuk mengedit data task
export const updateTask = createAsyncThunk(
  "task/updateTask",
  async (payload, { dispatch }) => {
    try {
      const response = await axios.put(
        "https://pm-todo-api.informatika-itts.my.id/api/tasks/" + payload.id,
        payload
      );
      const results = response.data;
      if (results.success) {
        dispatch(
          fetchTasks({
            nim: payload.nim,
            isComplete: payload.completed ? "1" : "0",
          })
        );
      }
      return results;
